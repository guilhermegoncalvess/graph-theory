...
Edge* graphEdges(Graph* G)
{
  ...
  for( v = 0; v < G->V; v++ )
  {
    ...
    while ( w != NULL )
    {
      edges[e].firstVertex = v;
      edges[e].secondVertex = w->node;
...
int main()
{
  int nVertex, nEdgeG, nEdgeH, firstVertex, secondVertex, weight, i=0;
  ...
  while (scanf("%d%d%d", &firstVertex, &secondVertex, &weight) != EOF)
  {
      if( i < nEdgeG )
          insertEdge( G, firstVertex, secondVertex, weight );
      else
          insertEdge( H, firstVertex, secondVertex, weight );
      i++;
  }
  ...
  for ( i = 0; i < nEdgeG; i++ )
  {
    printf("%d ", edgesG[i].firstVertex);
    printf("%d ", edgesG[i].secondVertex);
  ...
  for ( i = 0; i < nEdgeH; i++ )
  {
    printf("%d ", edgesH[i].firstVertex);
    printf("%d ", edgesH[i].secondVertex);
